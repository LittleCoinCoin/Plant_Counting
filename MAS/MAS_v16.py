...
,        """
    ...
    def Save_MetaSimulation_Results(self):
        ...
        """
        name = self.Make_File_Name("MetaSimulationResults_v16_nu06_"+self.simu_name)
    ...
    def Save_RALs_Infos(self):
        ...
        meta_simulation_results dictionary as a JSON file.
        """
        name = self.Make_File_Name("RALs_Infos_v16_nu06_"+self.simu_name)
        file = open(self.path_output+"/"+name+".json", "w")
        json.dump(self.RALs_data, file, indent = 2)
        file.close()
    
    def Save_Whole_Field_Results(self):
        name = self.Make_File_Name("WholeFieldResults_v16_nu06_"+self.simu_name)
        file = open(self.path_output+"/"+name+".json", "w")
        json.dump(self.whole_field_counted_plants, file, indent = 2)
        file.close()
    
    def Save_Log(self):
        name = self.Make_File_Name("LOG_MetaSimulationResults_v16_nu06_"+self.simu_name)
