...
v18 :
    -new agent plant growing policy
...
class ReactiveAgent(object):
    ...
    def Exploring_Intention(self, _x_dir, _y_dir):
        """
        _x_dir & _x_dir are relative positions to the RA
        
        Compares its pixel state to the pixel at coord (_x_dir, _y_dir)
        
        If RA is Black & the Direction is White --> Explore (0)
        If RA is White & the Direction is White --> Explore (0)
        If RA is White & the Direction is Black --> Stay (1)
        If RA is Black & the Direction is Black --> Shrink (2)
        
        If the pixel is already outside the frame of the image -- > Shrink (2)
        
        If the Direction is not in the frame of the image array --> Stay (1)
        """
        decision = 1
        ...
        if (not self.outside_frame):
            ...
            x_dir_global = self.global_x + _x_dir
            y_dir_global = self.global_y + _y_dir
            
            if (0<= y_dir_global <self.img_array.shape[0] and
                0<= x_dir_global <self.img_array.shape[1]):
                
                _dir_global_state = False
                if (self.img_array[y_dir_global, x_dir_global][0] > 220):
                    _dir_global_state = True
                
                if (not self.decision and _dir_global_state):
                    decision = 0
                    
                elif (self.decision and _dir_global_state):
                    decision = 0
                    
                elif (self.decision and not _dir_global_state):
                    decision = 1
                    
                else:# (not self.decision and not _dir_global_state):
                    decision = 2
        else:
            decision = 2
...
# =============================================================================
    ...
    def RAs_border_init(self):
        ...
        self.RA_list_card = self.Add_One_Layer_of_RAs(self.Borders_Distance)
        
# =============================================================================
#         #North Border
#         self.RA_list_card += [[ReactiveAgent(self.x, self.y, i, self.group_size, self.img_array) for i in range(-self.group_size,
#                                                                                                    self.group_size+self.group_step,
#                                                                                                    self.group_step)]]
#         #South Border
#         self.RA_list_card += [[ReactiveAgent(self.x, self.y, i, -self.group_size, self.img_array) for i in range(-self.group_size,
...
#                                                                                                     self.group_step)]]
#         #East Border                                                                                
#         self.RA_list_card += [[ReactiveAgent(self.x, self.y, self.group_size, j, self.img_array) for j in range(-self.group_size,
#                                                                                                    self.group_size+self.group_step,
#                                                                                                    self.group_step)]]
#         #West Broder                                                                                           
#         self.RA_list_card += [[ReactiveAgent(self.x, self.y, -self.group_size, j, self.img_array) for j in range(-self.group_size,
...
# =============================================================================
    ...
    def Get_RAs_Mean_Point(self):
        ...
        try:
            ...
            print("self.recorded_Decision_Score:", self.recorded_Decision_Score)
            print("self.active_RA_counter:", active_RA_counter)
            print("self.RA_list", self.RA_list)
            print("self.nb_RAs:", self.nb_RAs)
            print("self.RA_list_card", self.RA_list_card)
            print("nb_outside_frame_RAs", nb_outside_frame_RAs)
        
        if (active_RA_counter != 0):
            self.active_RA_Point[0] = mean_x/active_RA_counter
            self.active_RA_Point[1] = mean_y/active_RA_counter  
    
    def Get_RAs_Mean_Point_2(self):
        """
        ...
        for _RA in self.RA_list_card[0][1:-1]:
            for k in range (-self.group_step,
                            ...
                            -self.group_step):
                if (_RA.Explore_Pixel(_RA.global_x, _RA.global_y + k)):
                    mean_x += _RA.global_x
                    mean_y += _RA.global_y + k
                    active_RA_counter += 1
            else:
                nb_outside_frame_RAs += 1
        
        try:
            self.recorded_Decision_Score += [active_RA_counter/
                                             ((self.Borders_Distance[0]+abs(self.Borders_Distance[1]-2))*
                                              (self.Borders_Distance[2]+abs(self.Borders_Distance[3]-2)))]
        except ZeroDivisionError:
            print("self.recorded_Decision_Score:", self.recorded_Decision_Score)
            print("self.active_RA_counter:", active_RA_counter)
            print("self.RA_list", self.RA_list)
    ...
    def Manage_RAs_distribution(self):
        """
        Decides whether the RAL grows or shrinks
        """
# =============================================================================
#         print("RAL_ coordinates", self.x, self.y)
#         print("Manage Distribution")
# =============================================================================
        
        _decision_list = [1,1,1,1]#North, South, East, West
        _decision_dict = {"0":0, "1":0, "2":0}
        
        _decision_list[0] = self.Border_Movement_Decision(0, 0, 1)
        _decision_dict[str(_decision_list[0])]+=1
        
        _decision_list[1] = self.Border_Movement_Decision(1, 0, -1)
        _decision_dict[str(_decision_list[1])]+=1
        
        _decision_list[2] = self.Border_Movement_Decision(2, 1, 0)
        _decision_dict[str(_decision_list[2])]+=1
        
        _decision_list[3] = self.Border_Movement_Decision(3, -1, 0)
        _decision_dict[str(_decision_list[3])]+=1
        
        
        print("_decision_list:", _decision_list)
        print("_decision_dict:", _decision_dict)
        if (_decision_dict["0"] > 2):#majority in favor of exploration
            print("Grow")
            self.Grow()
        
        else:
            for i in range(4):
                if (_decision_list[i] == 2):
                    print("i", i, "Shrink")
                    self.Shrink(i)
        
        self.RA_list = self.RA_list_card[0]+self.RA_list_card[1]+\
                        self.RA_list_card[2]+self.RA_list_card[3]
        
        self.nb_RAs = len(self.RA_list)
        
        #self.Debug_RA_Distribution()
        
        print("self.Borders_Distance:", self.Borders_Distance)
        print("RAL_ coordinates", self.x, self.y)
        
    def Border_Movement_Decision(self, _border_index, _x_dir, _y_dir):
        """
        For One border, we ask all the pixel agents whether they wish to 
        - Explore (0)
        - Stay (1)
        - Shrink (2)
        
        We vote for the decision
        In case of equality, the choice is Explore > Stay > Shrink
        """
        _decision = 1
        _decision_list = [0,0,0]
        for _RA in self.RA_list_card[_border_index]:
            _decision_list[_RA.Exploring_Intention(_x_dir, _y_dir)] += 1
        
        _max_vote = max(_decision_list)
        if (_decision_list[0] == _max_vote):
            _decision = 0
        elif (_decision_list[2] == _max_vote):
            _decision = 2
        
# =============================================================================
#         print("Border Decision", _border_index, _decision_list)
# =============================================================================
        return _decision
    
    def Shrink(self, _border_index, _inc = 1):
        """
        Retracts a specific border. 
        We decrement the positions of the RAs. And we shrink the borders that
        are contiguous
        """
        #North
        if (_border_index == 0):
            self.Borders_Distance[0] -= _inc * self.group_step
            for _RA in self.RA_list_card[0]:
                _RA.Update_All_coords(0, -_inc * self.group_step)#Update the coordinates
                _RA.Is_Inside_Image_Frame()
            
            #We shrink East and West
            self.RA_list_card[2] = self.RA_list_card[2][:-_inc]
            self.RA_list_card[3] = self.RA_list_card[3][:-_inc]
        
        #South
        if (_border_index == 1):
            self.Borders_Distance[1] += _inc * self.group_step
            for _RA in self.RA_list_card[1]:
                _RA.Update_All_coords(0, _inc * self.group_step)#Update the coordinates
                _RA.Is_Inside_Image_Frame()
        ...
        #East
        if (_border_index == 2):
            self.Borders_Distance[2] -= _inc * self.group_step
            for _RA in self.RA_list_card[2]:
                _RA.Update_All_coords(-_inc * self.group_step, 0)#Update the coordinates
                _RA.Is_Inside_Image_Frame()
            
            #We shrink North and south
            self.RA_list_card[0] = self.RA_list_card[0][:-_inc]
            self.RA_list_card[1] = self.RA_list_card[1][:-_inc]
        
        #West
        if (_border_index == 3):
            self.Borders_Distance[3] += _inc * self.group_step
            for _RA in self.RA_list_card[3]:
                _RA.Update_All_coords(_inc * self.group_step, 0)#Update the coordinates
                _RA.Is_Inside_Image_Frame()
    ...
    def Grow(self, _inc = 10):
        """
        increase the size of the RAL in all four directions.
        This means adding new RAs at the beginning and end of each lists and
        incrementing their position.
        """
        
        #we update the distance of the Border relatively to the center
        
        
        
        Positive_Extension = [i * self.group_step for i in range (1, _inc+1)]
        Negative_Extension = [i * self.group_step for i in range (-_inc, 0)]
        
        #When growing, we do not care in which order we add the new Reactive agents.
        ...
        self.RA_list_card[0] = [ReactiveAgent(self.x, self.y,
                                        self.Borders_Distance[3]+k, self.Borders_Distance[0]+_inc * self.group_step,
                                        self.img_array) for k in Negative_Extension] + \
                            self.RA_list_card[0] + \
                           [ReactiveAgent(self.x, self.y,
                                        self.Borders_Distance[2]+k, self.Borders_Distance[0]+_inc * self.group_step,
                                        self.img_array) for k in Positive_Extension] 
        ...
        self.RA_list_card[1] = [ReactiveAgent(self.x, self.y,
                                        self.Borders_Distance[3]+k, self.Borders_Distance[1]-_inc * self.group_step,
                                        self.img_array)for k in Negative_Extension] + \
                            self.RA_list_card[1] + \
                           [ReactiveAgent(self.x, self.y,
                                        self.Borders_Distance[2]+k, self.Borders_Distance[1]-_inc * self.group_step,
                                        self.img_array) for k in Positive_Extension]
        ...
        self.RA_list_card[2] = [ReactiveAgent(self.x, self.y,
                                        self.Borders_Distance[2]+_inc * self.group_step, self.Borders_Distance[1]+k,
                                        self.img_array) for k in Negative_Extension] + \
                            self.RA_list_card[2] + \
                           [ReactiveAgent(self.x, self.y,
                                        self.Borders_Distance[2]+_inc * self.group_step, self.Borders_Distance[0]+k,
                                        self.img_array) for k in Positive_Extension]
        #West
        for _RA in self.RA_list_card[3]:
            _RA.Update_All_coords( -_inc * self.group_step, 0)#Update the coordinates
        ...
        self.RA_list_card[3] = [ReactiveAgent(self.x, self.y,
                                        self.Borders_Distance[3]-_inc * self.group_step, self.Borders_Distance[1]+k,
                                        self.img_array) for k in Negative_Extension] + \
                            self.RA_list_card[3] + \
                           [ReactiveAgent(self.x, self.y,
                                        self.Borders_Distance[3]-_inc * self.group_step, self.Borders_Distance[0]+k,
                                        self.img_array) for k in Positive_Extension] 
    ...
    def Debug_RA_Distribution(self):
        surface_print=np.zeros((self.Borders_Distance[0]+1+abs(self.Borders_Distance[1]),
                                self.Borders_Distance[2]+1+abs(self.Borders_Distance[3])))
        
        for _RA in self.RA_list:
            surface_print[_RA.local_y+abs(self.Borders_Distance[1]),
                          _RA.local_x+abs(self.Borders_Distance[3])] = 1
...
#         print()
# =============================================================================
        for _plant_pred in self.plant_FT_pred_in_crop_row[:1]:
...
# =============================================================================
    ...
    def Initialize_RowAs(self):
        ...
        for _crop_row in self.plant_FT_pred_par_crop_rows[1:2]:
...
class Simulation_MAS(object):
    ...
    def Perform_Simulation_Simple(self, _steps = 10,
                                      _coerced_X = False,
                                      _coerced_Y = False,
                                      _analyse_and_remove_Rows = False,
                                      _edge_exploration = False):
        
        print("Starting MAS Simple Simu:")
        self.steps = _steps
        self.max_steps_reached = False
    ...
    def Show_RALs_Position(self,
        ...
        for _RowsA in self.AD.RowAs:
            for _RAL in _RowsA.RALs:
                for _RA in _RAL.RA_list_card[0][1:-1]:
                    for k in range (-_RAL.group_step,
                                    -_RAL.Borders_Distance[0]+_RAL.Borders_Distance[1],
                                    -_RAL.group_step):
                        circle = patches.Circle((_RA.global_x,_RA.global_y+k),
                                            radius = 0.5,
                                            linewidth = 2,
                                            edgecolor = None,
                                            facecolor = 'orange')
